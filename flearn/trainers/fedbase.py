...
class BaseFedarated(object):
    def __init__(self, params, learner, dataset, p_generator = None, descending = False):
    ...
    def aggregate_ps(self, weights, losses, samples):
        base = [0]*len(weights[0])
        demominator = np.sum(samples)
        if self.sampling == 2:    
            samples =  [demominator for i in range(0, len(samples))]

        num_clients = len(weights)
        ps = self.p_generator(num_clients)
        weighted_weights = []
        for client_weights, sample_size in zip(weights,samples):
            weighted_weights.append([layer * sample_size / demominator for layer in client_weights])

        zip_weight_losses = list(zip(weighted_weights, losses))

        zip_weight_losses.sort(key= lambda tu: tu[1], reverse=self.descending)


        sorted_weights = [tu[0] for tu in  zip_weight_losses]
        
        for client_weights, p in zip(sorted_weights,ps):
            new_weights = [layer * p for layer in client_weights]
    ...
    def aggregate2(self, weights_before, Deltas, hs): 
        ...
        for client_delta in Deltas:
            weighted_weights.append([layer * 1.0 / demominator for layer in client_delta])

        updates = []
        for i in range(len(Deltas[0])):
            tmp = weighted_weights[0][i]
            for j in range(1, len(Deltas)):
                tmp += weighted_weights[j][i]
            updates.append(tmp)
